...
headers = {"Content-Type": "application/json",
           ...
           "X-Auth-Token": "gAAAAABgXcs-sGZ1guPuMsp3KN5QIx9EWhdmEMrGQpdUDCINOLH7W5yFX0veeCGcZjNPua4jTgvZpzzYFkrqr06eeRR7MzkiGZg4KN8dSckgcDHqUueqH0YaPFLfG0RiqGiQ2908VQ4XSvNcIos-V8LM9yQqK6MrHkDLs1AAsOZvDnvGtI03cpk"
           }


def request_image(url):
    '''请求镜像列表API获取json数据保存至文件'''
...
def image_excel(file):
    ...
    # 保存数据
    wb.save('glance镜像信息表.xls')
    print("成功写入excel表格！")


def generate_uuid_to_name():
    '''根据镜像名字，查找到对应的uuid，并附加到前一列'''
    ...
    for sheet in sheets:
        ...
        print(f'{sheet}加载成功！')
        print(range(1, image_sheet.nrows))
        for row in range(1, image_sheet.nrows):
            list.append(image_sheet.cell(row, 1).value)

        # 在glance镜像表中遍历查找镜像名对应的uuid
        glance_info_sheet = xlrd.open_workbook('鹏城实验室glance镜像信息查询/glance镜像信息表.xls').sheet_by_index(0)
...
def main():
    # request_image(url)
    # image_excel('images.json')
    generate_uuid_to_name()

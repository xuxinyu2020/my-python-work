...
def deal_disk_static(app_id):
    ...
    if res_util["cellList"] is None:
        ...
        if type(disk_instance) is dict:
            ...
            print(dict_temp)
            all_disk += int(dict_temp['DiskSize'])
            all_disk_used += int(dict_temp['DiskUsed'])
            all_disk_util = '%.2f%%' % ((all_disk_used / all_disk) * 100)
            result[1].append(dict_temp['DiskName'] + ' ' + dict_temp['DiskUtilization'] + '%')
            result[2].append(dict_temp['DiskName'] + ' ' + StrOfSize(int(dict_temp['DiskUsed'])))
            result[3].append(dict_temp['DiskName'] + ' ' + StrOfSize(int(dict_temp['DiskFreeSpace'])))
